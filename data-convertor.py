...
cluster = Cluster(['hostname'], port=9042, auth_provider=auth_provider)
session = cluster.connect('books_reviews')
...
for data_chunk in pd.read_csv('../data/data.tsv', sep='\t', chunksize=c_size, error_bad_lines=False, encoding='utf8'):
    ...
    for index, row in data_chunk.iterrows():
        ...
        try:
            session.execute(
                """
                INSERT INTO top_N_products_by_date (review_date, customer_id, reviews_amount)
                VALUES (%s, %s, %s)
                IF NOT EXISTS
                """, 
                (row['review_date'], row['product_id'], row['reviews_amount'])
            )
            print(row['review_date'], ' ', index)
        except cassandra.InvalidRequest:
            pass
    ...
    for index, row in data_chunk.iterrows():
        ...
        try:
            session.execute(
                """
                INSERT INTO top_N_products_by_date (review_date, customer_id, bad_reviews_amount)
                VALUES (%s, %s, %s)
                IF NOT EXISTS
                """, 
                (row['review_date'], row['product_id'], row['bad_reviews_amount'])
            )
            print(row['review_date'], ' ', index)
        except cassandra.InvalidRequest:
            pass
    ...
    for index, row in data_chunk.iterrows():
        ...
        try:
            session.execute(
                """
                INSERT INTO top_N_products_by_date (review_date, customer_id, good_reviews_amount)
                VALUES (%s, %s, %s)
                IF NOT EXISTS
                """, 
                (row['review_date'], row['product_id'], row['good_reviews_amount'])
            )
            print(row['review_date'], ' ', index)
        except cassandra.InvalidRequest:
            pass
